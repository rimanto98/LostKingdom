...
func _process(delta):
	if called:
		get_node("/root/Minigame1/Pause3D/PauseLayer").add_child(guide_node)
		called = false
		emit_signal("guide",true)
		
	if Input.is_action_just_released("ui_cancel") && get_node("/root/Minigame1/Pause3D/PauseLayer/Pause").visible:
		get_node("/root/Minigame1/Pause3D/PauseLayer").remove_child(guide_node)
